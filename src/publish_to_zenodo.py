...
def publish_preprint(md_path, pdf_path, token):
    ...
    data = {
        "metadata": {
            ...
            "description": f"Meta-análisis científico: {first_line}",
...
def main():
    ...
    for md_file in md_files:
        topic = md_file.replace("Chapter_", "").replace(".md", "").split("_", 1)[1]
        pdf_file = f"Preprint_Neuro_{topic}_JuanMoisésdelaSerna.pdf"
        md_path = os.path.join(preprints_dir, md_file)
        pdf_path = os.path.join(preprints_dir, pdf_file)

        if os.path.exists(pdf_path):
            print(f"Publishing {md_file} to Zenodo...")
            doi = publish_preprint(md_path, pdf_path, token)
            if doi:
